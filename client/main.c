...
void check_game_changes()
{
    ...
    for (int i = 0; i < current_state.num_players; i++)
    {
        ...
        {
            if (i == my_player_id)
            { // Chỉ thông báo cho người chơi hiện tại
...
int main(int argc, char *argv[])
{
    ...
    inet_pton(AF_INET, "172.18.30.238", &serv_addr.sin_addr);
        ...
        while (SDL_PollEvent(&e))
        {
            ...
            case SCREEN_LOGIN:
                ...
                if (e.type == SDL_TEXTINPUT)
                {
                    if (inp_user.is_active)
                        handle_text_input(&inp_user, e.text.text[0]);
                    if (inp_pass.is_active)
                        handle_text_input(&inp_pass, e.text.text[0]);
                }
                ...
                if (e.type == SDL_KEYDOWN)
                {
                    ...
                    {
                        if (inp_user.is_active)
                            handle_text_input(&inp_user, '\b');
                        if (inp_pass.is_active)
                            handle_text_input(&inp_pass, '\b');
                    }
            ...
            case SCREEN_REGISTER:
                ...
                if (e.type == SDL_TEXTINPUT)
                {
                    if (inp_user.is_active)
                        handle_text_input(&inp_user, e.text.text[0]);
                    if (inp_email.is_active)
                        handle_text_input(&inp_email, e.text.text[0]);
                    if (inp_pass.is_active)
                        handle_text_input(&inp_pass, e.text.text[0]);
                }
            ...
            case SCREEN_LOBBY_LIST:
                ...
                if (show_create_room_dialog)
                {
                    ...
                    if (e.type == SDL_TEXTINPUT)
                    {
                        if (inp_room_name.is_active)
                            handle_text_input(&inp_room_name, e.text.text[0]);
                    ...
                    if (e.type == SDL_KEYDOWN)
                    {
                        ...
                        {
                            if (inp_room_name.is_active)
                                handle_text_input(&inp_room_name, '\b');
                            if (inp_access_code.is_active)
                                handle_text_input(&inp_access_code, '\b');
                        }
